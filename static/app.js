...
function copyNumber(num) {
    ...
    navigator.clipboard.writeText(num).then(() => {
        ...
        t.innerText = "Номер скопійовано";
        document.body.appendChild(t);
        setTimeout(() => t.remove(), 1200);
    });
}

function showFlashMessage(text, duration = 1000) {
...
async function saveCandidate(obj) {
    try {
        const res = await fetch("/api/candidates", {
            method: "POST",
    ...
    if (list.length === 0) {
        ...
        list.forEach(c => {
            ...
            card.innerHTML = `
                ...
                <div class="meta">${c.experience || ""}</div>
                <div class="note">${c.myNote || ""}</div>
...
function updateNotifications() {
    ...
    let html = "";
    if (toCall.length === 0) {
        html += `<div class="notif-empty">✅ Усі обдзвонені</div>`;
    } else {
        html += `<h4>📞 Потрібно подзвонити (${toCall.length}):</h4>`;
        toCall.forEach(c => {
            const isToday = c.callbackDate && c.callbackDate === today;
            html += `
                <div class="notif-item" onclick="openDetail(${c.id});" title="Відкрити анкету">
    ...
    if (interning.length > 0) {
        ...
        interning.forEach(c => {
            html += `<div class="notif-item small" onclick="openDetail(${c.id});">${c.name} • ${c.city || ""}</div>`;
        });
    }
...
// ---------------- INIT ----------------
loadCandidates();
setInterval(() => {}, 60 * 1000); // placeholder minimal reminders
